...
## Migrating for the grouped cellar display and atomic multi-bottle add

Adds one new RPC, `register_bottles_from_cellar_group` (see README "Personal Cellar" — "Atomic multi-bottle add") — no new table, column, or RLS policy, and no change to any existing cellar RPC. It generalizes `register_bottle_from_cellar` (unchanged, still used for one-bottle groups) to atomically reserve N matching bottles and create N tasting bottles in a single transaction.
...
1. **`p_cellar_bottle_id` is only an anchor** — one bottle the client already legitimately has from its own RLS-scoped fetch. The function never trusts a client-supplied "group" of ids; it re-derives the full matching set itself from `(owner_user_id = auth.uid(), status = 'available', <every material identity field equal to the anchor's>)`, using `is not distinct from` throughout so `null` fields (e.g. no appellation) compare correctly.
2. **Locking and determinism**: the matching-rows subquery is `order by created_at asc, id asc ... limit p_quantity for update` — the same "oldest first, id as a stable tie-breaker" rule the grouped display's client-side helper (`lib/cellarGrouping.ts`'s `oldestBottleFirst`) documents, and `for update` means a concurrent identical call blocks rather than double-reserving the same row.
3. **All-or-nothing**: if fewer than `p_quantity` matching rows can be locked, the function raises `cellar_group_stock_changed` *before* any insert/update runs — no partial reservation, no partial tasting-bottle creation, ever.
4. **One transaction, N wines, N reservations**: a single `insert ... select ... returning` creates every wine row (continuing the session's existing `bottle_number`/`tasting_order`/`display_order`/`anonymous_code` allocation scheme), and a chained `update ... from inserted` reserves exactly the corresponding cellar rows, preserving the same 1:1 `wines.cellar_bottle_id` linkage `register_bottle_from_cellar` already established.
5. **Granted to `authenticated` only** (not `anon`), matching every other cellar-mutating RPC — a signed-out request can never reach this far, since `auth.uid()` would already be null and the function raises immediately.
...
where specific_name = (
  select specific_name from information_schema.routines
  where routine_name = 'register_bottles_from_cellar_group'
) order by ordinal_position;

-- requesting more bottles than currently available fails safely, with no
-- partial insert/reservation left behind
select register_bottles_from_cellar_group(
  (select guest_token from guests limit 1),
  (select id from cellar_bottles where status = 'available' limit 1),
  9999
);
-- expect: an error containing 'cellar_group_stock_changed', and no new
-- rows in `wines` or status changes in `cellar_bottles` as a result
```

### RLS and privacy summary

No RLS policy changed and no new grant was added on `cellar_bottles` or `wines` themselves. `register_bottles_from_cellar_group` runs as `SECURITY DEFINER`, re-validates the caller's `auth.uid()` and cellar ownership independently of anything the client claims, and only ever operates on rows it has itself just re-selected and locked — a stale, tampered, or otherwise-incorrect client-side "group" can never cause it to touch a bottle outside the caller's own current matching set. The response contains only `{ids, count, firstBottleNumber}` — `ids` are the new wines' own ids (never rendered to the user), and no cellar bottle id, note, or other private field is ever returned.

## Migrating for host-controlled results reveal

Adds one new RPC (`reveal_full_blind_bottle`), two new host-only read RPCs (`get_bottle_result_for_host`, `get_provisional_leaderboard_for_host`), one new guest-only read RPC (`get_revealed_bottles_summary`), replaces `get_revealed_bottle` in place with a mode-aware, own-guess-only version, and adds `contributorName` to `get_host_session`'s existing non-seen wine list — see README "Results reveal". No new table, column, or RLS policy; no existing reveal/lock/scoring behaviour changed for course_reveal.

### 1. Run the SQL (already correct if you paste the whole file)

```sql
-- Host-controlled results reveal (see README "Results reveal"): full_blind's
-- per-bottle equivalent of reveal_bottle, but deliberately WITHOUT that
-- function's "must be the single earliest-unrevealed bottle" ordering check —
-- full_blind's own spec explicitly allows revealing eligible bottles in any
-- order, unlike course_reveal's fixed serving sequence. Idempotent
-- (`coalesce(revealed_at, now())`, matching reveal_seen_ratings's pattern)
-- rather than erroring on an already-revealed bottle, so a host's browser
-- retry or double-click can never fail loudly. Once every bottle in the
-- session has been individually revealed, this flips
-- tasting_sessions.status to 'revealed' itself (the same tail reveal_bottle
-- already uses for course_reveal) — this is what makes the existing,
-- completely unmodified /results/[publicId] final report/leaderboard page
-- become reachable automatically the moment the host finishes revealing
-- every bottle, for full_blind exactly as it already does for course_reveal.
create or replace function reveal_full_blind_bottle(
  p_public_id uuid,
  p_host_token text,
  p_wine_id uuid
) returns jsonb
...
begin
  ...
  return jsonb_build_object('wineId', p_wine_id, 'revealedAt', v_revealed_at, 'sessionRevealed', v_session_revealed);
end;
$$;

-- Guest: fetch the reveal for one specific bottle — the answer key,
-- contributor name, and the CALLER'S OWN guess only (not every guest's) —
-- once that bottle's revealed_at is actually set.
--
-- MIGRATION-SENSITIVE BEHAVIOR CHANGE: this function previously returned
-- *every* guest's locked guess for the bottle, visible to any participant.
-- The new host-controlled results reveal feature's privacy model requires an
-- ordinary participant see only their own score breakdown — every other
-- guest's guess is now host-only (see get_bottle_result_for_host below).
-- `guesses` is kept as a 0-or-1-element array (rather than becoming a bare
-- nullable object) so existing callers that map over it (see
-- buildRevealedBottleResult in lib/supabase/mappers.ts) needed no shape
-- change, only a narrower result. Also newly mode-aware — previously
-- course_reveal-only in practice (via wine_guesses.locked_at, always null
-- for full_blind); now also serves full_blind bottles once
-- reveal_full_blind_bottle sets their revealed_at, using the exact same
-- per-mode "submitted" predicate get_bottle_response_progress already
-- established.
...
begin
  ...
  if v_session.tasting_mode = 'full_blind' then
    v_submitted := v_guest.completed_at is not null;
  else
    select (locked_at is not null) into v_submitted
      from wine_guesses where wine_id = v_wine.id and guest_id = v_guest.id;
    v_submitted := coalesce(v_submitted, false);
  end if;
  ...
  select jsonb_build_object(
    ...
    'guesses', case when v_submitted then coalesce((
      ...
      where wg.wine_id = v_wine.id and wg.guest_id = v_guest.id
...
-- Host: the per-participant score-breakdown counterpart to get_revealed_bottle
-- above — same revealed_at gate and same per-mode "submitted" predicate, but
-- host-token-authenticated and returning EVERY eligible participant's guess
-- (never just the caller's own). A participant with no submitted guess still
-- appears in `participants` with `submitted: false` and `guess: null`,
-- rather than being silently dropped. Never returns a token, email, or any
-- other participant field beyond their display name.
create or replace function get_bottle_result_for_host(
  p_public_id uuid,
  p_host_token text,
  p_wine_id uuid
) returns jsonb
...
-- Bottle numbers and contributor names are never secret at any point in this
-- app (see README "Bottle-order contributor labels") — only actual wine
-- identity is, and this function never returns any answer-key field.
create or replace function get_revealed_bottles_summary(
  p_guest_token text
) returns jsonb
language plpgsql
security definer
...
begin
  ...
  select
    ...
    count(*),
    count(*) filter (where w.revealed_at is not null)
  into v_bottles, v_total_count, v_revealed_count
  from wines w where w.session_id = v_session.id;

  return jsonb_build_object(
    'bottles', v_bottles,
    'allRevealed', v_session.status = 'revealed',
...
-- Host: the provisional-leaderboard data source — every currently-revealed
-- bottle's full answer key plus every guest's guess for those bottles only.
-- Deliberately returns raw rows rather than computing ranks here: the client
-- builds a synthetic TastingSession scoped to just these revealed wines and
-- calls the exact same buildRevealedSubmissions/buildCourseRevealSubmissions
-- + calculateTasterResults pipeline the final /results page already uses
-- (see lib/resultsReveal.ts) — no ranking/scoring math is duplicated for
-- this "revealed bottles only" scope, only the data query changes. A still-
-- unrevealed bottle's row (and its guesses) are never selected at all, not
-- merely filtered out client-side.
create or replace function get_provisional_leaderboard_for_host(
  p_public_id uuid,
  p_host_token text
) returns jsonb
language plpgsql
...
-- get_revealed_bottle's existing grant needs no change — its signature is unchanged:
-- grant execute on function get_revealed_bottle(text, uuid) to anon, authenticated;
```

Also add `contributorName` to `get_host_session`'s existing non-seen wine list (the `else` branch of its mode check) — find this block and add the one new field:

```sql
-- contributorName is included here for the same reason
-- get_active_bottle_state/get_guest_session_state already include it for
-- participants — bottle numbers and contributor names are never secret at
-- any point in this app, only actual wine identity is. This lets the new
-- full_blind per-bottle Results reveal list show the same safe label
-- participants already see during guessing.
select coalesce(jsonb_agg(jsonb_build_object(
  'id', w.id,
  'bottleNumber', w.bottle_number,
  'anonymousCode', w.anonymous_code,
...
select routine_name from information_schema.routines
where routine_name in (
  'reveal_full_blind_bottle', 'get_bottle_result_for_host',
  'get_revealed_bottles_summary', 'get_provisional_leaderboard_for_host'
);

-- get_revealed_bottle now rejects an unrevealed bottle and, once revealed,
-- returns 0-or-1 guesses (the caller's own only) plus a `submitted` flag
select get_revealed_bottle(
  (select guest_token from guests limit 1),
  (select id from wines where revealed_at is null limit 1)
);
-- expect: an error containing 'bottle_not_revealed'

-- a host attempting the per-bottle host result view for a not-yet-revealed
-- bottle is rejected the same way a participant would be
select get_bottle_result_for_host(
  (select public_id from tasting_sessions limit 1),
  '<the session''s host token>',
  (select id from wines where revealed_at is null limit 1)
);
-- expect: an error containing 'bottle_not_revealed'
```
...
## Migrating for Bottle photos

Adds an optional photo per tasting-registration bottle (see README "Bottle photos") — a new public Storage bucket, one new column, one new RPC, additive output fields on every RPC/view that already returns wine identity, and — for the first time in this app — a Supabase service-role key, used in exactly one narrow place.

### 1. Run the SQL (already correct if you paste the whole file)

Re-pasting `supabase/schema.sql` in full is the complete database migration:

1. **`wines.photo_path text`** is added (`add column if not exists`), plus a length check constraint (`wines_photo_path_length`, `photo_path is null or length(photo_path) <= 300`), guarded the same `do $$ ... $$` way as `wines_appellation_length` before it.
2. **The `bottle-photos` Storage bucket** is created via `insert into storage.buckets (...) on conflict (id) do nothing` — public, an 8MB `file_size_limit`, and `allowed_mime_types` restricted to `image/jpeg`, `image/png`, `image/webp`. One `storage.objects` policy, `bottle_photos_public_read`, allows `select` for anyone against that bucket — there is deliberately no `insert`/`update`/`delete` policy for `anon`/`authenticated`; every write goes through the service-role client instead (see below), which bypasses RLS entirely.
3. **`authorize_bottle_photo_upload(p_guest_token text, p_mime_type text)`** is a new `SECURITY DEFINER` RPC: validates the guest token (plain equality against `guests.guest_token`, same as every other guest RPC), rejects unless the session is still `status = 'registration'`, validates the mime type against the same three-value whitelist server-side, and returns a server-generated object path (`{session_id}/{guest_id}/{uuid}.{ext}`) — never a client-supplied path or extension.
4. **`register_bottle` and `update_bottle` are modified** — same pattern as every prior signature change in this file: the old-arity overloads are explicitly `drop function if exists`'d first, then recreated with a new trailing `p_photo_path text default null` parameter. Both additionally validate any submitted path starts with the exact `{session_id}/{guest_id}/` prefix as defense-in-depth, on top of the RPC in step 3 already only ever generating paths in that shape.
5. **Additive output fields**, all named `photoPath`: `get_registration_state`'s `myBottles`; `get_revealed_bottle` and `get_bottle_result_for_host`'s wine object; `get_provisional_leaderboard_for_host` and `get_final_leaderboard_for_guest`'s wine objects; `get_seen_tasting_state`'s bottle objects (unconditional — Seen shows identity unmasked from the start, same as every other field there); and `get_host_session`'s nested `seen` block (the host-side mirror of the same always-visible Seen treatment).
6. **`guest_visible_wines` view** gains a trailing masked `photo_path` column, following the exact same `case when s.status = 'revealed' then w.photo_path else null end` pattern already used for every other identity column.
7. **Grants**: `execute` on `authorize_bottle_photo_upload` to `anon, authenticated` (it only ever needs a guest token, exactly like every other guest-facing RPC); the existing `register_bottle`/`update_bottle` grant lines are updated to match their new parameter-type lists. `wines`' direct `anon`/`authenticated` column grant is **not** changed — it stays exactly `(id, session_id, bottle_number, anonymous_code, created_at)`, so `photo_path` only ever reaches a client through the gated RPCs/view above, exactly like every other answer-key field.

### 2. Set the service-role key

This is the one place in this app that uses the Supabase **service-role** key, because Storage's Row Level Security can only evaluate `auth.uid()`/JWT claims — and a tasting-registration contributor is an anonymous bearer-token caller with no `auth.uid()`, so there's no way for Storage RLS to check their guest token directly the way `authorize_bottle_photo_upload` (a database function, which *can* just query `guests` directly) already does.

1. In the Supabase dashboard, go to **Project Settings** → **API**, and copy the **`service_role`** key (not the `anon`/`public` key you copied in step 2 of the main setup above).
2. Add it to `.env.local` as `SUPABASE_SERVICE_ROLE_KEY` — **never** prefix it `NEXT_PUBLIC_`, and never paste it anywhere else. It is read only by `lib/supabase/serviceClient.ts`, which is only ever imported from `app/api/register/photo-upload-url/route.ts` (a server-only Route Handler) — it never reaches a Client Component or the browser bundle.
3. Restart your dev server (or redeploy) after adding it — like every other env var in this app, it's only read at server start.

If this key is left unset, bottle photo uploads fail with a clear "Photo uploads aren't configured" error rather than a crash; every other feature in this app is completely unaffected, since nothing else reads this env var.

### 3. Verify the bucket

The SQL in step 1 creates the `bottle-photos` bucket for you — this is just a sanity check:

1. In the dashboard, go to **Storage**. Confirm a `bottle-photos` bucket exists and is marked **Public**.
2. Click into it → **Policies**, and confirm exactly one policy exists (`bottle_photos_public_read`, `SELECT`) — no insert/update/delete policy should be present for any role.
...
- **No service-role key for any tasting read or write.** Every privileged tasting operation goes through a Postgres function (`SECURITY DEFINER`) that checks the host/guest token *inside the database* before doing anything. The app's Next.js server and browser code use the public **anon** key for every one of those — this remains true after adding Supabase Auth, which also never uses a service-role key (see "Accounts and the `authenticated` role" below). All four tasting tables have Row Level Security enabled, and neither `anon` nor `authenticated` has direct table grants beyond a few already-public columns (used only so Realtime has something safe to broadcast — for `wines`, that's just `id`, `session_id`, `bottle_number`, `anonymous_code`, `created_at`; every answer-key column and `contributor_guest_id` are excluded). Everything else — the actual wine answer keys, contributor identity, everyone's guesses — is only reachable through the views/functions in `schema.sql`, which enforce the "don't leak the answer key or contributor identity before reveal" rule. The **one exception** in this whole app is bottle photos (see "Migrating for Bottle photos" above): a service-role key mints a Storage signed-upload URL inside exactly one Route Handler, after that Route Handler has already independently checked the caller's guest token via the normal anon-key client — it has no elevated access to any tasting table, and no other code path in this app reads that key at all.
