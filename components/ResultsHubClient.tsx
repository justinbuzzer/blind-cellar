...
interface ResultsHubClientProps {
  /** Where each revealed bottle's row links — the mode-appropriate per-bottle reveal/result page. */
  resultHref: (wineId: string) => string;
  /** Where "View final leaderboard" points once every bottle is revealed. */
  leaderboardHref: string;
  /** Where "View tasting recap" points once every bottle is revealed. */
  recapHref: string;
}

/**
 * Participant results hub — see README "Results reveal". Shared by full_blind
 * (/tasting/[publicId]/results) and course_reveal (/session/[publicId]/results),
 * which differ only in their per-bottle/leaderboard/recap hrefs. Live updates
 * use the same realtime-signal-plus-poll-fallback convention as
 * app/session/[publicId]/active/page.tsx and app/tasting/[publicId]/page.tsx.
 */
export function ResultsHubClient({ resultHref, leaderboardHref, recapHref }: ResultsHubClientProps) {
