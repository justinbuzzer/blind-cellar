...
import { friendlyRpcError, RevealSeenRatingsResponse } from "@/lib/supabase/types";

interface RevealSeenRatingsBody {
  publicId: string;
  hostToken: string;
  wineId: string;
}
...
export async function POST(request: NextRequest) {
  ...
  let body: RevealSeenRatingsBody;
  try {
  ...
  const { data, error } = await supabase.rpc("reveal_seen_ratings", {
  ...
  return NextResponse.json(data as RevealSeenRatingsResponse);
}
