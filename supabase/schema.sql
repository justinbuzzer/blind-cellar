...
begin
  ...
  if v_session.tasting_mode = 'seen' then
    ...
    -- participants (see README "Bottle-order contributor labels") — bottle
    -- numbers and contributor names are never secret at any point in this
    -- app, only actual wine identity is. This lets the new full_blind
    -- per-bottle Results reveal list (see README "Results reveal") show the
    -- same safe label participants already see during guessing.
    select coalesce(jsonb_agg(jsonb_build_object(
      'id', w.id,
      'bottleNumber', w.bottle_number,
      'anonymousCode', w.anonymous_code,
...
-- per-bottle equivalent of reveal_bottle above, but deliberately WITHOUT that
...
-- already uses for course_reveal) — this is deliberate and is what makes the
-- existing, completely unmodified /results/[publicId] final report/
-- leaderboard page become reachable automatically the moment the host
-- finishes revealing every bottle, for full_blind exactly as it already does
-- for course_reveal. reveal_tasting_session above still exists and is
-- untouched, but the new per-bottle host UI never calls it again.
...
-- once that bottle's revealed_at is actually set. This is the sole path to
-- this data during 'collecting': it is never included in
-- get_active_bottle_state, any pre-reveal view, or the anon column grant on
-- wines/wine_guesses.
--
-- MIGRATION-SENSITIVE BEHAVIOR CHANGE (see README "Results reveal"): this
-- function previously returned *every* guest's locked guess for the bottle,
-- visible to any participant. The new host-controlled results reveal
-- feature's privacy model requires an ordinary participant see only their
-- own score breakdown — every other guest's guess is now host-only (see
-- get_bottle_result_for_host below). `guesses` is kept as a 0-or-1-element
-- array (rather than becoming a bare nullable object) so existing callers
-- that map over it (see buildRevealedBottleResult in
-- lib/supabase/mappers.ts) needed no shape change, only a narrower result.
-- Also newly mode-aware — previously course_reveal-only in practice (via
-- wine_guesses.locked_at, always null for full_blind); now also serves
-- full_blind bottles once reveal_full_blind_bottle sets their revealed_at,
-- using the exact same per-mode "submitted" predicate
-- get_bottle_response_progress already established (full_blind:
-- guests.completed_at; course_reveal: wine_guesses.locked_at) so the two
-- functions' definitions of "submitted" can never quietly diverge.
...
-- (never just the caller's own), since only the host is allowed to see
-- everyone's breakdown for a bottle (see README "Results reveal"). Eligible
-- participants are every row in `guests` for the session (the same
-- denominator get_bottle_response_progress and get_host_guess_progress
-- already use — the host is counted at most once, as an ordinary guest row,
-- never double-counted). A participant with no submitted guess still
-- appears in `participants` with `submitted: false` and `guess: null`,
-- rather than being silently dropped, so the host's table can show
-- "No submitted guess" for them without a second query. Never returns a
-- token, email, or any other participant field beyond their display name.
...
-- (session status reaching 'revealed' — see reveal_full_blind_bottle and
-- reveal_bottle above, both of which flip it automatically on the final
-- reveal). Bottle numbers and contributor names are never secret at any
-- point in this app (see README "Bottle-order contributor labels") — only
-- actual wine identity is, and this function never returns any answer-key
-- field. This is what lets a participant's results page list every bottle
-- (so they can navigate to whichever ones are already revealed) without
-- exposing anything about a still-unrevealed bottle beyond its existence
-- and position, which participants already see during guessing anyway.
...
-- (see README "Results reveal"). Deliberately returns raw rows rather than
-- computing ranks here: the client builds a synthetic TastingSession scoped
-- to just these revealed wines and calls the exact same
-- buildRevealedSubmissions/buildCourseRevealSubmissions +
-- calculateTasterResults pipeline the final /results page already uses (see
-- lib/supabase/mappers.ts, lib/results.ts) — no ranking/scoring math is
-- duplicated for this "revealed bottles only" scope, only the data query
-- changes. A still-unrevealed bottle's row (and its guesses) are never
-- selected at all, not merely filtered out client-side.
